...
def matchSpikeToPeak(trace, spikes, search_win_size = 3):
    ...
    search_win_half_size = np.int(2 * np.floor(search_win_size/2))
...
def verifySpikeSTD(trace, spikes, detrend_win_size = 100, spike_win_size = 10, thredshold = 2):
    ...
    detrend_win_half_size = np.int(np.floor(detrend_win_size/2))
    ...
    spike_mask[spikes] = 1
    spike_mask = np.convolve(spike_mask, np.ones((np.int(2*np.floor(spike_win_size/2) + 1),)), 'same')
